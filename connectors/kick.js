...
// ---------------------------------------------------------------------------
// 🟢 v0.169.3 → 🛡️ v0.172: como falar com a API do site do Kick (kick.com/api/...)
//
...
// quando — e passou a levar quase sempre. Os caminhos de reserva (Node com a
// assinatura do Chrome → curl → PowerShell → navegador aberto) moram agora em
// connectors/caminhos.js, compartilhados com as outras redes; o Kick é a
// única com o caminho 5 (o site aceita a consulta feita por uma página).
// ---------------------------------------------------------------------------
const KICK_API = process.env.OBS_SOCIAL_KICK_API || 'https://kick.com/api/';
const caminhos = criarCaminhos({
  rede: 'kick', rotulo: 'Kick', bloqueador: 'o Cloudflare', navegador: true,
  headers: BROWSER_HEADERS, referer: 'https://kick.com/',
});
const kickApiConfigurar = caminhos.configurar;
const kickApiEstado = caminhos.estado;

// Devolve { status, ok, texto, json, via } (ver caminhos.js). `extra: true` =
// pedido que não é essencial (avatar, audiência): quando tudo acabou de ser
// barrado, desiste na hora em vez de insistir.
function kickApi(caminho, opcoes = {}) {
